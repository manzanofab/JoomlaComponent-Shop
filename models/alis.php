...
/**
 * Methods supporting a list of ali records.
 *
 * @package		Joomla
 * @subpackage	com_alis
 */
class AlisModelAlis extends JModelList
{
	/**
	 * Constructor.
	 *
...
//echo nl2br(str_replace('#__','foo_',$query));
	...
	public function getAuthors() {
		...
		$query->join('INNER', '#__alis AS c ON c.created_by = u.id');
		$query->group('u.id');
		$query->order('u.name');
	...
	public function getItems()
	{
		...
		if ($app->isSite()) {
			...
			for ($x = 0, $count = count($items); $x < $count; $x++) {
				//Check the access level. Remove alis the user shouldn't see
